...
import utils.FeedbackCalculation
from utils.GetAction import get_action, select_action, act_restriction_FLOPs
from utils.NN2Graph import *
from models.Encoder_GCN import *
from models.Decoder import Decoder_Env_LSTM as Env
...
from utils.NN2Graph import *
from utils.SplitDataset import get_split_valset_ImageNet, get_split_valset_CIFAR, get_split_train_valset_CIFAR
...
def trainer():
    ...
    #utils.FeedbackCalculation.ErrorCaculation_CIFAR(net, device,'DNN/datasets')

    n_layer=55
    FLOPs = utils.FeedbackCalculation.FlopsCaculation(net, 32, 32)
    print("total FLOPs:",sum(FLOPs))
    #50% Flops reduction
    desired_FLOPs = sum(FLOPs[:]) * 0.2
    ...
    for episode in range(500):
        ...
        s = G_embedding
        s, rewards = env(s,torch.randn([1,1,1]).cuda(), 0)
        #print("s_size",s.shape)
        for t in range(n_layer):
            #print("s device",s.device)
            a = agent.act(s)
            #a = select_action(a,episode)

            #a,reduced = get_action(torch.tensor(a), t, desired_FLOPs, reduced, FLOPs)
            #a_list.append(a.numpy())
            a_list.append(a)
            #print("a",torch.Tensor(a))
            a=a.reshape([1,1,1])
            s_, rewards = env(s,torch.tensor(a).cuda(),t)
            if t == n_layer-1:
                a_list = act_restriction_FLOPs(a_list, net, desired_FLOPs, FLOPs)
                rewards = utils.FeedbackCalculation.RewardCaculation(a_list, n_layer, net, FLOPs,device)
                rewards = float(rewards)
            else:
                rewards=0
            agent.put(s.squeeze(0),a.squeeze(0).squeeze(0),rewards,s_.squeeze(0))
...
def train(num_episode, agent, env, output,G,net,n_layer,graph_encoder,train_loader,val_loader):
    ...
    while episode < num_episode:  # counting based on episode
        ...
        if done:  # end of episode
            ...
            #print(a_list)
            rewards,best_accuracy = RewardCaculation_FineTune(a_list, n_layer, net, FLOPs, best_accuracy,train_loader,val_loader,device,root='DNN/datasets/')
...
if __name__ == "__main__":
    ...
    #device = torch.device("cpu")
    Motifs_embedding = torch.randn([4, 50])
    G = ResNet2Graph(Motifs_embedding)
    G.to(device)
    ...
    #checkpoint = torch.load('DNN/pretrained_models/resnet20-12fca82f.th',map_location=device)
    checkpoint = torch.load('DNN/pretrained_models/resnet56-4bfd9763.th', map_location=device)
    net.load_state_dict(checkpoint['state_dict'])
    ...
    n_layer = i
    print("n_conv_layer:",i)

    FLOPs = FlopsCaculation(net, 32, 32)
    print("total FLOPs:", sum(FLOPs))
    # 50% Flops reduction
    desired_FLOPs = sum(FLOPs[:]) * 0.6
    ...
    train_loader, val_loader, n_class = get_split_train_valset_CIFAR('cifar10', 256, 4, 5000,1000, data_root='DNN/datasets', use_real_val=False,
                           shuffle=True)
    train(args.train_episode, agent, env , args.output,G,net,n_layer,graph_encoder,train_loader,val_loader)
