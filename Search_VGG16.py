...
def train(num_episode, agent, env, output,G,net,n_layer,graph_encoder,val_loader):
    ...
    while episode < num_episode:  # counting based on episode
        ...
        if done:  # end of episode
            ...
            #print(a_list)
            rewards,best_accuracy = RewardCaculation_ImageNet(a_list, n_layer, net, FLOPs, best_accuracy,train_loader,val_loader,device,root='DNN/datasets/')
...
if __name__ == "__main__":
    ...
    n_layer = i
    FLOPs = FlopsCaculation(net, 224, 224)
    print("total FLOPs:", sum(FLOPs))
    # 50% Flops reduction
    desired_FLOPs = sum(FLOPs[:]) * 0.7
    ...
    train_loader, val_loader, n_class = get_split_valset_ImageNet("ImageNet", 128, 4,1000, 1000, data_root='DNN/datasets/',
