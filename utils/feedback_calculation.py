...
def flops_caculation_forward(net, args, input_x, a_list=None):
    ...
    if 'resnet' in args.model:
        ...
        if a_list is not None:
            ...
            from utils.get_action import act_share
        ...
        if a_list is not None:
            ...
            flops[::2] = flops[::2] * np.append([1],np.array(a_list[:-1]).reshape(-1)).reshape(-1)
        ...
        for name,layer in net.named_modules():
            if isinstance(layer,torch.nn.Conv2d):
                flop, input_x = layer_flops(layer, input_x)
                flops.append(flop)
        if a_list is not None:
            # prune mobilenet block together(share pruning index between depth-wise and point-wise conv)

            if len(flops[1::3]) != len(a_list):
                print(len(flops[1::3]))
                raise IndexError

            flops_ratio=[]
            for i in range(len(a_list)):
                flops_ratio.append(a_list[i])
                flops_ratio.append(a_list[i])
                flops_ratio.append(a_list[i])
            flops_ratio.append(1)


            flops = flops * np.array(flops_ratio).reshape(-1)
            #
            # flops[::2] = flops[::2] * np.array(a_list).reshape(-1)
            # flops[::2] = flops[::2] * np.append([1],np.array(a_list[:-1]).reshape(-1)).reshape(-1)
            #
...
def error_caculation(model, val_loader, device, finetune=False, train_loader=None):
    ...
    if finetune and train_loader != None:
        dataloaders = {}
        ...
        print("Fine Tuning ...................")
        model, top1, top5 = \
            train_model_top5(model, dataloaders, criterion, optimizer_ft, device, num_epochs=1, is_inception=False)
...
def reward_caculation(args, a_list, DNN, best_accuracy, val_loader,train_loader=None, root='./logs'):
    # TODO here we do the pseudo pruning, by zero mask those pruned channel
    new_net = network_pruning(DNN, a_list, args)
    device = torch.device(args.device)

    if args.dataset == 'cifar10':
        val_top1, val_top5 = error_caculation(new_net, val_loader, device)
        acc = val_top1

    elif args.dataset == 'ILSVRC':
        # new_net,val_top1, val_top5 = error_caculation(new_net, val_loader, device,True,train_loader)
        val_top1, val_top5 = error_caculation(new_net, val_loader, device,False,train_loader)
        acc = val_top5
